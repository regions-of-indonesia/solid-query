...
const createSolidQuery = (client: RegionsOfIndonesiaClient = new RegionsOfIndonesiaClient()) => {
  const key = {
      provinces: ["provinces"],
  ...
  return {
    ...
    createProvince: (code?: string) =>
      createQuery(() => initialDataOptions(getValidKey(code, key.province), fetcher.province, isKey(code))),
    ...
    createDistrict: (code?: string) =>
      createQuery(() => initialDataOptions(getValidKey(code, key.district), fetcher.district, isKey(code))),
